...
const TablePg = () =>{
    ...
    const addColumn=()=>{
        ...
        const type=prompt('Enter column type(text, number, email, data , checkbox, dropdown, textarea, url, phone, currency):','text');
        ...
        if (type==='dropdown'){
            const opts=prompt('Enter dropdown options(comma seperated):', 'Option1, Option2');
                ...
                <table>
                    ...
                    <tbody>
                        {rows.map((row, rowIndex)=>(
                                         ...
                                         {col.type === 'textarea' ? (
                                            ...
                                            <div style={{display:'flex', alignItems:'center'}}>
                                                <input type={col.type === 'phone' ? 'tel' : col.type === 'currency' ? 'number' : col.type} value={row[col.label] || ''} onChange={(e) => {  
                                                    let value = e.target.value; if (col.type === 'number' || col.type === 'currency') {
                                                    value = Number(value);
                                                  }
                                                     handleCellChange(rowIndex, col.label, value);
                                                }}/>
                                                {col.type === 'currency' && (<span style={{ marginLeft: '6px', fontWeight: 'bold', color: '#4CAF50', }}title={col.currency}>  {getCurrencySymbol(col.currency)}  </span>)}
                            ...
                            <td>
                                <button onClick={()=> deleteRow(rowIndex)} className='delete-btn'> delete </button>
                            </td>
                            </tr>
                        ))}
                    </tbody>
